...
function dePalabrasAFrase(palabras) {
  ...
  // Tu código:
  let nuevoArray = palabras.join()
  let nuevoArray2 = nuevoArray.replaceAll("," , " ")
  return nuevoArray2
}
...
function cuentoElementos(arreglo){
  ...
  for (n of arreglo)
  {
    if (n>18)
    {
      longitudArreglo.push(n)
    }
  }
  return longitudArreglo.length
...
function diaDeLaSemana(numeroDeDia) {
  ...
  function empiezaConNueve(numero) {
    ...
    let final = numerostring.charAt(0)
    if (final == 9)
    return true
    else {
      return false
...
function todosIguales(arreglo) {
  ...
  //Escribe tu código aquí  
  let resultado = arreglo.filter((item,index) => {
...
function mesesDelAño(array) {
  ...
  // Tu código:
  let nuevoArray = []
  let mesOk = array.find((mes) => mes === "Enero" || mes === "Marzo" || mes === "Noviembre")
  nuevoArray.push(mesOk)
